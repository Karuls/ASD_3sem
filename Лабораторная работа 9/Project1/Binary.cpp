...
int populationSize;
int child;
int evoCount; // кол-во поколнений
...
bool repeat(string s, char ch) { // проверка, содержится ли символ
    return s.find(ch) != string::npos;
}

string create_gnome() { // создание случайного маршрута 
    string gnome = "0";
...
int cal_fitness(string gnome) { // вычисление длины маршрута
    int f = 0;
...
string crossover(string parent1, string parent2) { // два маршрута в один
...
string mutatedGene(string gnome) { // мутация маршрута, перестановка двух городов с вероятностью mutationRate
...
void Genetic_algo() {
    ...
    for (int i = 0; i < populationSize; i++) {
        individual temp;
        temp.gnome = create_gnome();// сл. маршрк
        temp.fitness = cal_fitness(temp.gnome); // приспосоленность
    ...
    while (gen <= evoCount) {
        ...
        for (int i = 0; i < child; i++) {
            ...
            individual new_ind; // Новая популяция объединяется с текущей
